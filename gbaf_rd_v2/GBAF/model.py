...
from GBAF.utils import PositionalEncoder, Power_reallocate, Power_reallocate_fb
from GBAF.gbaf_components import Encoder, Decoder


class GBAF(nn.Module):
    def __init__(self, args):
        super(GBAF, self).__init__()
        self.args = args
        self.pe = PositionalEncoder()

        # Transmitter
        self.Tmodel = Encoder(args.clas + (1 + args.num_feedback) * (args.T - 1), args.m, args.attention_size, args.num_layers_T,
                              args.num_heads_att, args.dropout)
        # Feedback
        if args.gbaf_active_feedback == True:
            self.Fmodel = Encoder(args.T - 1 + args.num_feedback * (args.T - 2), args.m, args.attention_size,
                                  args.num_layers_F, args.num_heads_att, args.dropout, args.num_feedback)
        # Receiver
        self.Rmodel = Decoder(args.T, args.m, args.attention_size, args.num_layers_R, args.num_heads_att,
    ...
    def power_constraint_T(self, inputs, isTraining, step, idx=0):
        if isTraining == 1:
            ...
            if step == 0:
                ...
                torch.save(this_mean, 'G/this_mean_T' + str(idx))
                torch.save(this_std, 'G/this_std_T' + str(idx))
            elif step <= 100 :
                this_mean = (torch.load('G/this_mean_T' + str(idx)) + torch.mean(inputs, 0)) / 2
                this_std = (torch.load('G/this_std_T' + str(idx)) + torch.std(inputs, 0)) / 2

                torch.save(this_mean, 'G/this_mean_T' + str(idx))
                torch.save(this_std, 'G/this_std_T' + str(idx))
            else:
                this_mean = torch.load('G/this_mean_T' + str(idx))
                this_std = torch.load('G/this_std_T' + str(idx))
    ...
    def power_constraint_F(self, inputs, isTraining, step, idx=0):
        if isTraining == 1:
            ...
            if step == 0:
                ...
                torch.save(this_mean, 'G/this_mean_F' + str(idx))
                torch.save(this_std, 'G/this_std_F' + str(idx))
            elif step <= 100 :
                this_mean = (torch.load('G/this_mean_F' + str(idx)) + torch.mean(inputs, 0)) / 2
                this_std = (torch.load('G/this_std_F' + str(idx)) + torch.std(inputs, 0)) / 2

                torch.save(this_mean, 'G/this_mean_F' + str(idx))
                torch.save(this_std, 'G/this_std_F' + str(idx))
            else:
                this_mean = torch.load('G/this_mean_F' + str(idx))
                this_std = torch.load('G/this_std_F' + str(idx))

        outputs = (inputs - this_mean) * 1.0 / (this_std + 1e-8)
        return outputs


    def forward(self, step, bVec_md, fwd_noise_par, fb_noise_par, table=None, isTraining=1):
        ...
        for t in range(self.args.T):
            ...
            if t == 0:
                x_T = torch.cat([bVec_md, torch.zeros(self.args.batchSize, self.args.l, (1 + self.args.num_feedback) * (self.args.T - 1)).cuda()], dim=2)
                ...
                # Generate feedback
                if self.args.gbaf_active_feedback == False:
                    feedback = self.power_constraint_F(received, isTraining, step, t) + fb_noise_par[:, :, t, :]
                else:  # active feedback
                    x_F = torch.cat([received, torch.zeros(self.args.batchSize, self.args.l, self.args.num_feedback * (self.args.T - 2) + self.args.T -2).cuda()], dim=2)
                ...
                x_T = torch.cat([bVec_md, parity_all,
                ...
                if self.args.gbaf_active_feedback == False:
                    ...
                    x_F = torch.cat([received, parity_fb], dim=2)
                    output_F = self.Fmodel(x_F, None, self.pe)
                    output_F = self.power_constraint_F(output_F, isTraining, step, t)
                    if self.args.reloc == 1:
                        output_F = self.total_power_reloc2(output_F, t)
                ...
                x_T = torch.cat([bVec_md, parity_all, feedback], dim=2)
                output_T = self.Tmodel(x_T, None, self.pe)
                parity = self.power_constraint_T(output_T, isTraining, step, t)
                if self.args.reloc == 1:
                    parity = self.total_power_reloc1(parity, t)
        ...
        output = self.Rmodel(received, None, self.pe)
        return output
